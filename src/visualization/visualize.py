...
def calculate_outliers(df, num_cols, figures_dir):
    ...
    pd.DataFrame(outlier_stats).sort_values(by='Porcentaje_Outliers', ascending=False).to_csv(
        f"{figures_dir}/outlier_stats.csv", index=False
    )
...
def plot_scatter_pairs(df, pairs, figures_dir):
    for x, y in pairs:
        if x in df.columns and y in df.columns:
            ...
            sns.scatterplot(data=sample, x=x, y=y, alpha=0.5, edgecolor=None, s=20)
            sns.regplot(data=sample, x=x, y=y, scatter=False, color='red', ci=None, line_kws={'lw':1})
...
def main(input_path="data/clean/steel_energy_clean.csv", figures_dir="reports/figures"):
    ...
    if "Usage_kWh" in df.columns:
        for cat in ["WeekStatus", "Day_of_week", "Load_Type"]:
            if cat in df.columns:
                bar_by_category(df, "Usage_kWh", cat, figures_dir, agg="mean")
