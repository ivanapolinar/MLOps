...
class MLOpsPipeline:
    """Pipeline MLOps completo: limpieza, split, entrenamiento, tuning, evaluación y guardado."""
    ...
    def process_raw_to_clean(self, input_filepath: str, output_filepath: str) -> pd.DataFrame:
    ...
    def train_base(self, X_train, y_train, preprocessing, params: Optional[dict] = None):
    ...
    def export_feature_importance(self, model, num_cols, cat_cols, figures_dir: str):
    ...
    def run_training_pipeline(
        ...
        with mlflow.start_run():
            # Base model
            base_params = {}
            base_model = self.train_base(X_train, y_train, preprocessing, base_params)
            mlflow.log_params(base_params)

            acc_base, fig_base, _ = self.evaluate(base_model, X_test, y_test, figures_dir, name="base")
            mlflow.log_metric("base_accuracy", acc_base)
            if fig_base:
                mlflow.log_artifact(fig_base)

            # Hyperparameter tuning
            best_model, best_params = self.tune_hyperparams(base_model, X_train, y_train)
            mlflow.log_params(best_params)

            acc_opt, fig_opt, _ = self.evaluate(best_model, X_test, y_test, figures_dir, name="optimized")
            ...
            # Feature importances
            fi_csv, top_png = self.export_feature_importance(best_model, num_cols, cat_cols, figures_dir)
            ...
            # Log MLflow model (artifact only, NO 'name')
            X_example = X_test[:2].copy()
            if len(X_example.select_dtypes(include="integer").columns) > 0:
                X_example = X_example.astype("float64")
